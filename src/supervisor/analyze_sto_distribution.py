...
from clients.so_prepare_bubble import AnalyzeLevel2Demand
...
class Analysis:
    ...
    def _get_client(self, item):
        return AnalyzeLevel2Demand(item.get("Code"),item.get("ClientCode"), item.get("WarehouseCode"), item.get("WarehouseSiteId"),item.get("CreatedBy"),item.get("IsTruckingPlan"),item.get("Type"))
    ...
    def finished(self, id):
        self.db.getCollection(os.getenv("BD_COLLECTION_DEMAND_SET")).update_one(
            {"_id": id}, {"$set": {"Status": constant.STATUS_ANALYZED}})
...
def load_queue():
    _db = db()
    filters = {
        "Name": {"$in" : ["ANALYZE_STO_DISTRIBUTION","REANALYZE_STO_DISTRIBUTION"]},
        "Status": {"$in": [constant.STATUS_NEW]},
        "IsDeleted": 0
    }
    results = []
    ...
    for item in cursor:
        results.append({
            ...
            "Code": item.get("Code"),
            "CreatedBy": item.get("CreatedBy"),
            "IsTruckingPlan": item.get("IsTruckingPlan")
        })
    _db.close()
    return results
