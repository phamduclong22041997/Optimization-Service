...
from clients.so_bubble import AnalyzeLevel3Demand
...
class Analysis:
    ...
    def _get_client(self, item):
        return AnalyzeLevel3Demand(item.get("ClientCode") ,item.get("Code") ,item.get("WarehouseCode"), item.get("WarehouseSiteId"),item.get("CreatedBy"),item.get('Name'))
    ...
    def finished(self, id):
        self.db.getCollection(os.getenv("BD_COLLECTION_DEMAND_SET")).update_one(
            {"_id": id,"Status" : {"$nin": [constant.STATUS_COMPLETED,constant.WAITING_FOR_CREATE_SO]}}, {"$set": {"Status": constant.STATUS_ANALYZED}})
...
def load_queue():
    _db = db()
    filters = {
        "Name": {"$in" : ["ANALYZE_SO_AUTOMATION","ANALYZE_SO_INVENTORY"]} ,
        "Status": {"$in": [constant.STATUS_NEW]},
        "IsDeleted": 0
    }
    results = []
